...
class SummarizerAgent(BaseAgent):
    ...
    def process(self, stream: str, payload: dict):
        ...
        if event == "fetch.completed":
            ...
            if user_id <= 0 and not content_ids:
                return None
            self._summarize_pending(user_id=user_id, content_ids=content_ids)
    ...
    # -------- Summarization --------
    def _summarize_pending(self, user_id: int, content_ids: list[int]):
        ...
        with get_session() as session:
            ...
            message_bus.emit_summary_started(user_id=user_id, content_count=total)

            for content in contents:
                try:
                    start = time.perf_counter()
                    result = self._run_sync(content)
        ...
        message_bus.emit_summary_completed(
            user_id=user_id, summary_ids=new_ids, count=len(new_ids),
        )
        logger.info(
    ...
    def _run_sync(self, content: ContentModel) -> LLMResult:
